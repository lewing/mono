	...
	internal class Startup {
		...
		public void Configure (IApplicationBuilder app, IHostingEnvironment env)
		{
			...
			app.UseDeveloperExceptionPage ()
				.UseWebSockets ()
				.UseMonoWasmDebugProxy ();
		}
	}

	static class MonoWasmDebugExtensions {
		public static BrowserTab MonoWasmDebugTapMapper (BrowserTab tab, HttpContext context, Uri debuggerHost)
		{
		...
		public static IApplicationBuilder UseMonoWasmDebugProxy (this IApplicationBuilder app)
		{
			return UseMonoWasmDebugProxy (app, new MonoWasmDebugSettings {
				DebuggerHost = new Uri ("http://localhost:9222"),
				TabMapper = MonoWasmDebugTapMapper
			});
		}

		public static IApplicationBuilder UseMonoWasmDebugProxy (this IApplicationBuilder app, MonoWasmDebugSettings settings)
		{
			app.Use(async (context, next) => {
				var rewriteFunc = settings.TabMapper;
				var debuggerHost = settings.DebuggerHost;
				var request = context.Request;
				...
				switch (requestPath.Value.ToLower (System.Globalization.CultureInfo.InvariantCulture)) {
					...
					case "/json/version":
						...
						break;
					case "/json/new":
						var tab = await ProxyGetJsonAsync <BrowserTab> (endpoint);
					...
					case "/json/list":
					case "/json":
						var tabs = await ProxyGetJsonAsync <BrowserTab[]>(endpoint);
			...
			.UseRouter (router => {
					router.MapGet ("devtools/page/{pageId}", async context => {
						...
						var debuggerHost = settings.DebuggerHost;
						var endpoint = new Uri ($"ws://{debuggerHost.Authority}{context.Request.Path.ToString()}");
						try {
		...
		public class MonoWasmDebugSettings {
			public Uri DebuggerHost { get; set; }
			public Func<BrowserTab, HttpContext, Uri, BrowserTab> TabMapper { get; set; }
		}
